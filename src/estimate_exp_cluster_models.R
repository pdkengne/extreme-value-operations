estimate_exp_cluster_models <- function(x, cluster_data){
  # x:
  # cluster_data:
  
  if (length(cluster_data) == 1){
    cluster_models <- lapply(1:1, function(data){
      model <- fitdistrplus::fitdist(data = x, distr = "exp", method = "mle")
      model
    })
  }
  else {
    cluster_models <- lapply(cluster_data, function(data){
      model <- fitdistrplus::fitdist(data = data, distr = "exp", method = "mle")
      model
    })
  }
