...
calculate_laplace_mixture_model_inverse_cdf <- function(p, 
                                                        locations, 
                                                        scales,
                                                        weights, 
  ...
  if (is.element(el = kind, set = c("geometric", "arithmetic"))){
    output <- sapply(p, function(p){
      ...
      nle <- function(q){
        f <- -p + calculate_laplace_mixture_model_cdf(q = q, 
                                                      locations = locations, 
                                                      scales = scales, 
                                                      weights = weights,
                                                      kind = kind)
      ...
      q_initial_guesses <- sapply(1:length(weights), function(j) extraDistr::qlaplace(p = p, 
                                                                                      mu = locations[j], 
                                                                                      sigma = scales[j])) 
