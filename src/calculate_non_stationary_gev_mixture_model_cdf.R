...
calculate_non_stationary_gev_mixture_model_cdf <- function(ns_gev_mixture_model_object,
  ...
  #       note that the following condition must be satisfied nrow(data) == length(q)
  ...
  if (is.null(data)){
    data <- data.frame("x" = partial_data_covariates[partial_data > threshold, ])
  }
  ...
  if (nrow(data) != length(q) & nrow(data) > 1){
    stop("Sorry, the information in the input arguments must satisfy nrow(data) == length(q)!")
  }
