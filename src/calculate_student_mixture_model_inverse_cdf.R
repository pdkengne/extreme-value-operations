...
calculate_student_mixture_model_inverse_cdf <- function(p, 
                                                        locations, 
                                                        scales,
                                                        shapes,
                                                        weights, 
  ...
  if (is.element(el = kind, set = c("geometric", "arithmetic"))){
    output <- sapply(p, function(p){
      ...
      nle <- function(q){
        f <- -p + calculate_student_mixture_model_cdf(q = q, 
                                                      locations = locations, 
                                                      scales = scales, 
                                                      shapes = shapes,
                                                      weights = weights,
      ...
      q_initial_guesses <- sapply(1:length(weights), function(j) extraDistr::qlst(p = p, 
                                                                                  mu = locations[j], 
                                                                                  sigma = scales[j],
                                                                                  df = shapes[j])) 
...
# scales <- rexp(n = m)
# locations <- rnorm(n = m)
# shapes <- rpois(n = m, lambda = 10)
# 
# p <- seq(from = 0.01, to = 0.09, length.out = 9)
# 
# results <- calculate_student_mixture_model_inverse_cdf(p = p,
#                                                        locations,
#                                                        scales,
#                                                        shapes,
#                                                        weights,
...
# results
# 
# calculate_student_mixture_model_cdf(q = results,
#                                     locations,
#                                     scales,
#                                     shapes,
#                                     weights,
