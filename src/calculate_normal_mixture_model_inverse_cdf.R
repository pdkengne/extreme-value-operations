...
calculate_normal_mixture_model_inverse_cdf <- function(p, 
                                                       locations, 
                                                       scales,
                                                       weights, 
  ...
  if (is.element(el = kind, set = c("geometric", "arithmetic"))){
    output <- sapply(p, function(p){
      ...
      nle <- function(q){
        f <- -p + calculate_normal_mixture_model_cdf(q = q, 
                                                     locations = locations, 
                                                     scales = scales, 
                                                     weights = weights,
                                                     kind = kind)
      ...
      q_initial_guesses <- sapply(1:length(weights), function(j) qnorm(p = p, 
                                                                       mean = locations[j], 
                                                                       sd = scales[j])) 
...
# 
# results <- calculate_normal_mixture_model_inverse_cdf(p = p,
#                                                       locations,
#                                                       scales,
#                                                       weights,
#                                                       iterations = 100,
...
# results
# 
# calculate_normal_mixture_model_cdf(q = results,
#                                 locations,
#                                 scales,
#                                 weights,
#                                 kind = c("geometric", "arithmetic")[1])
