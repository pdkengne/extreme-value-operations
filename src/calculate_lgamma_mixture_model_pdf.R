...
calculate_lgamma_mixture_model_pdf <- function(x, 
                                               scales,
                                               shapes,
                                               weights, 
  ...
  if (kind == "geometric"){
    S <- sapply(1:length(weights), function(j){
      dens <- actuar::dlgamma(x = x,  
                              ratelog = scales[j], 
                              shapelog = shapes[j])
      
      prob <- actuar::plgamma(q = x, 
                              ratelog = scales[j], 
                              shapelog = shapes[j])
    ...
    cdf <- calculate_lgamma_mixture_model_cdf(q = x,  
                                              scales = scales, 
                                              shapes = shapes, 
                                              weights = weights,
                                              kind = kind)
...
# x <- 5
# 
# results <- calculate_lgamma_mixture_model_pdf(x = x,
#                                               scales,
#                                               shapes,
#                                               weights,
#                                               kind = c("geometric", "arithmetic")[1])
...
# 
# results_1 <- calculate_lgamma_mixture_model_pdf(x = x,
#                                                 scales,
#                                                 shapes,
#                                                 weights,
#                                                 kind = c("geometric", "arithmetic")[1])
...
# 
# 
# results_2 <- calculate_lgamma_mixture_model_pdf(x = x,
#                                                 scales,
#                                                 shapes,
#                                                 weights,
#                                                 kind = c("geometric", "arithmetic")[2])
