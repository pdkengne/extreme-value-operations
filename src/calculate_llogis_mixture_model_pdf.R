...
source("./src/calculate_llogis_mixture_model_cdf.R")

calculate_llogis_mixture_model_pdf <- function(x, 
                                               scales,
                                               shapes,
                                               weights, 
  ...
  if (kind == "geometric"){
    S <- sapply(1:length(weights), function(j){
      dens <- actuar::dllogis(x = x,  
                              scale = scales[j], 
                              shape = shapes[j])
      
      prob <- actuar::pllogis(q = x, 
                              scale = scales[j], 
                              shape = shapes[j])
    ...
    cdf <- calculate_llogis_mixture_model_cdf(q = x,  
                                              scales = scales, 
                                              shapes = shapes, 
                                              weights = weights,
                                              kind = kind)
...
# x <- 5
# 
# results <- calculate_llogis_mixture_model_pdf(x = x,
#                                               scales,
#                                               shapes,
#                                               weights,
#                                               kind = c("geometric", "arithmetic")[1])
...
# 
# results_1 <- calculate_llogis_mixture_model_pdf(x = x,
#                                                 scales,
#                                                 shapes,
#                                                 weights,
#                                                 kind = c("geometric", "arithmetic")[1])
...
# 
# 
# results_2 <- calculate_llogis_mixture_model_pdf(x = x,
#                                                 scales,
#                                                 shapes,
#                                                 weights,
#                                                 kind = c("geometric", "arithmetic")[2])
