source("./src/calculate_laplace_mixture_model_cdf.R")


calculate_laplace_mixture_model_pdf <- function(x, 
                                               locations, 
                                               scales, 
                                               weights,
  ...
  if (kind == "geometric"){
    S <- sapply(1:length(weights), function(j){
      dens <- extraDistr::dlaplace(x = x, 
                                   mu = locations[j], 
                                   sigma = scales[j])
      
      prob <- extraDistr::plaplace(q = x, 
                                   mu = locations[j], 
                                   sigma = scales[j])
    ...
    cdf <- calculate_laplace_mixture_model_cdf(q = x, 
                                               locations = locations, 
                                               scales = scales, 
                                               weights = weights,
                                               kind = kind)
...
# x <- 1:10
# 
# results <- calculate_laplace_mixture_model_pdf(x = x,
#                                                locations,
#                                                scales,
#                                                weights,
#                                                kind = c("geometric", "arithmetic")[1])
...
# 
# results_1 <- calculate_laplace_mixture_model_pdf(x = x,
#                                                  locations,
#                                                  scales,
#                                                  weights,
#                                                  kind = c("geometric", "arithmetic")[1])
...
# #results_1
# 
# results_2 <- calculate_laplace_mixture_model_pdf(x = x,
#                                                  locations,
#                                                  scales,
#                                                  weights,
#                                                  kind = c("geometric", "arithmetic")[2])
...
# locations <- c(-2, +2, +6)
# 
# x <- seq(from = -8, to = 12, length.out = 500)
# 
# results_1 <- calculate_laplace_mixture_model_pdf(x = x,
#                                                  locations,
#                                                  scales,
