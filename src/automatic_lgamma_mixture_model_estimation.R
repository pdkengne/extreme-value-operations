...
# generate data from a lgamma mixture model
library(mixR)
...
x = bmixture::rmixlgamma(n = 1000, weight = weight, alpha = alpha, beta = beta)
...
x <- rlgamma(n = 1000, shape = 9, scale = 1)
...
fit <- fitdist(data = x, distr = "lgamma")
...
cluster_models <- estimate_lgamma_cluster_models(cluster_data = cluster_attractors$cluster_data_list)
